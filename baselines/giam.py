from pettingzoo.mpe import simple_spread_v2, simple_push_v2 
...
parser.add_argument("--log_dir", type=str, default="./env-search")
args = parser.parse_known_args()[0] 

log_name = ["giam", args.env_name] 
...
self_agent_id = 1 # good agent 
other_agent_id = 0 # adversary agent 
...
giam_state_dim = self_state_dim + other_state_dim + other_action_dim # current O_self + current O_other + previous a_other (one hot vector) --> a_self 
giam_action_dim = self_action_dim 
...
for i_episode in range(1, hyperparams["max_episodes"]+1): 
    ...
    for t in range(1, hyperparams["max_cycles"]+1):
        action = {}
        giam_state = np.append(state[env.possible_agents[self_agent_id]], state[env.possible_agents[other_agent_id]])
        giam_state = np.append(giam_state, prev_a_other)

        action[env.possible_agents[self_agent_id]] = giam_agent.select_action(giam_state) 
        ...
        giam_agent.buffer.rewards.append(reward[env.possible_agents[self_agent_id]])
        giam_agent.buffer.is_terminals.append(done[env.possible_agents[self_agent_id]])
