...
giam_state_dim = self_state_dim # current O_self --> a_self 
...
for i_episode in range(1, hyperparams["max_episodes"]+1): 
    ...
    for t in range(1, hyperparams["max_cycles"]+1):
        action = {}
        action[env.possible_agents[self_agent_id]] = giam_agent.select_action(state[env.possible_agents[self_agent_id]]) 
