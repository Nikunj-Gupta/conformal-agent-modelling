...
log_name.append("z_dim" + str(args.z_dim)) 
...
ego_state_dim = state_dim + hyperparams["z_dim"] + action_dim 

cam_agent = CAM(
    state_dim=hyperparams["z_dim"], 
    act_dim=action_dim
) 
ego_agent = PPO(
    ego_state_dim, 
...
for i_episode in range(1, hyperparams["initial_start_episodes"]+1): 
    ...
    for t in range(1, hyperparams["max_cycles"]+1): 
        ...
        # Generate encoding of other agent's obs 
        mu, log_var = vae.encode(ego_agent.batchify_obs(state, agent_id=1).reshape(1, -1))
        z = vae.reparameterize(mu, log_var)
        z_other_obs = z.cpu().detach().numpy().reshape(-1) 
        ...
        cam_agent.buffer.states.append(z_other_obs)
