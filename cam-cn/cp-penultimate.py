...
parser.add_argument("--log_dir", type=str, default="./cp-reps")
args = parser.parse_known_args()[0] 

log_name = ["cam_plus_other_obs_plus_conformal_reps"]  
...
hyperparams = {
    ...
    "max_cycles":25,
    "update_timestep": 30, 
    "cp_update_timestep": int(args.cp_update_timestep), 
    "cp_rep_size": 64, 
...
cp_rep_size = hyperparams["cp_rep_size"] 
ego_state_dim = state_dim + state_dim + cp_rep_size 
...
for i_episode in range(1, hyperparams["initial_start_episodes"]+1): 
    ...
    for t in range(1, hyperparams["max_cycles"]+1): 
        action = {} 
        padded_S_pseudo = np.zeros(cp_rep_size)
        action[env.possible_agents[0]] = ego_agent.select_action(np.concatenate([
                state[env.possible_agents[0]], 
                state[env.possible_agents[1]], 
                padded_S_pseudo]
            )) 
...
for i_episode in range(1, hyperparams["max_episodes"]+1): 
    ...
    for t in range(1, hyperparams["max_cycles"]+1):
        ...
        output = cam_agent.get_conformal_action_representations(ego_agent.batchify_obs(state, agent_id=1).reshape(1, -1)) #TODO: Remove hardcoding of shape 
        # padded_S = np.pad(S[0], (0, action_dim-S[0].shape[0])) 
        """
        reps in padded S 
        """
        padded_S = output.cpu().detach().numpy().reshape(-1) 
        action[env.possible_agents[0]] = ego_agent.select_action(np.concatenate([
            state[env.possible_agents[0]], 
            state[env.possible_agents[1]], 
            padded_S] 
